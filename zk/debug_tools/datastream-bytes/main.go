...
func main() {
	...
	err := db.View(context.Background(), func(tx kv.Tx) error {
		...
		for _, blockNumber := range blocks {
			...
			sBytes, err := streamServer.CreateAndBuildStreamEntryBytesProto(block, hermezDb, tx, lastBlock, uint64(batchNum), uint64(previousBatch), l1InfoTreeMinTimestamps, nil)
			if err != nil {
				return err
			}
