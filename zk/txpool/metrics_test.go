	...
	for name, scenario := range scenarios {
		t.Run(name, func(t *testing.T) {
			...
			for i := uint64(0); i < scenario.firstRunTxIn; i++ {
				...
				pool.incrementAnnouncementToMetrics()
			}
			...
			pool.calculateTxInOut()
			assert.Equal(scenario.expectedFirstIn, pool.metrics.in)
			assert.Equal(scenario.expectedFirstOut, pool.metrics.out)
			...
			pool.calculateTxInOut()
			assert.Equal(scenario.expectedSecondRunIn, pool.metrics.in)
			assert.Equal(scenario.expectedSecondRunOut, pool.metrics.out)
		})
	}
}
