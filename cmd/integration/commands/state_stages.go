...
func syncBySmallSteps(db kv.RwDB, miningConfig params.MiningConfig, ctx context.Context) error {
	...
	execUntilFunc := func(execToBlock uint64) func(firstCycle bool, badBlockUnwind bool, stageState *stages.StageState, unwinder stages.Unwinder, tx kv.RwTx, quiet bool) error {
		return func(firstCycle bool, badBlockUnwind bool, s *stages.StageState, unwinder stages.Unwinder, tx kv.RwTx, quiet bool) error {
	...
	for (!backward && execAtBlock < stopAt) || (backward && execAtBlock > stopAt) {
		...
		if miner.MiningConfig.Enabled && nextBlock != nil && nextBlock.Coinbase() != (common2.Address{}) {
			...
			miningStages.MockExecFunc(stages.MiningCreateBlock, func(firstCycle bool, badBlockUnwind bool, s *stages.StageState, u stages.Unwinder, tx kv.RwTx, quiet bool) error {
...
func loopIh(db kv.RwDB, ctx context.Context, unwind uint64) error {
	...
	u := &stages.UnwindState{ID: stages.HashState, UnwindPoint: to}
	...
	u = &stages.UnwindState{ID: stages.IntermediateHashes, UnwindPoint: to}
...
func loopExec(db kv.RwDB, ctx context.Context, unwind uint64) error {
	...
	sync.MockExecFunc(stages.Execution, func(firstCycle bool, badBlockUnwind bool, stageState *stages.StageState, unwinder stages.Unwinder, tx kv.RwTx, quiet bool) error {
