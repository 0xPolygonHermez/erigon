...
1. Create 1 file for the dynamic config (example for Cardona in `zk/examples/dynamic-configs`, edit as required)
    - `dynamic-{network}-cfg.json` - the config file containing allocs, chainspec, and additional configuration
...
2. **_(optional)_** Use flag `zkevm.genesis-config-path` to specify the path to the dynamic config file. Doing this means you can name the file whatever you want, it's behavior overrides the `chain` field in the yaml file. Example: `--zkevm.genesis-config-path="/dynamic-mynetwork/mynetwork-cfg.json"`
