...
func New(stack *node.Node, config *ethconfig.Config) (*Ethereum, error) {
	...
	if backend.config.Zk != nil {
		...
		if httpCfg.DataStreamPort > 0 && httpCfg.DataStreamHost != "" {
			...
			backend.dataStream, err = datastreamer.NewServer(uint16(httpCfg.DataStreamPort), uint8(backend.config.DatastreamVersion), 1, datastreamer.StreamType(1), file, httpCfg.DataStreamWriteTimeout, 10*time.Second, 2*time.Second, logConfig)
			if err != nil {
				return nil, err
			}
