...
type Ethereum struct {
	...
	syncStages      []*stages.Stage
	syncUnwindOrder stages.UnwindOrder
	syncPruneOrder  stages.PruneOrder
...
func (backend *Ethereum) Init(stack *node.Node, config *ethconfig.Config) error {
	...
	backend.stagedSync = stages.New(backend.syncStages, backend.syncUnwindOrder, backend.syncPruneOrder)
